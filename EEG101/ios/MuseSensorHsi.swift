...
open class MuseSensorHsi: AWARESensor {
  static private let sensorName: String = "muse_hsi"
  static private let entityName: String = "EntityMuseHsi"
  
  private var isRecording: Bool = false
  var stream: MuseStreamHsi?
  ...
  public override init(awareStudy: AWAREStudy, dbType: AwareDBType) {
    ...
    let storage: AWAREStorage = SQLiteStorage.init(study: awareStudy, sensorName: MuseSensorHsi.sensorName, entityName: MuseSensorHsi.entityName, dbHandler: ExternalCoreDataHandler.shared(), insertCallBack: insertEntityCallback)

    // Prevent aware internal buffer as we already use our own ring buffer
    storage.setBufferSize(1)
    
    // Call new official initializer
    super.init(awareStudy: awareStudy, sensorName: MuseSensorHsi.sensorName, storage: storage)
  }
  ...
  open func startRecording(muse: IXNMuse) {
    do {
      ...
      self.stream = MuseStreamHsi(muse: muse)
  ...
  open override func createTable() {
    if self.isDebug() {
      os_log("[%@] Create Table", MuseSensorHsi.sensorName);
    }
