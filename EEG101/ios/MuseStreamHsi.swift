 ...
 * Not thread safe!
 **/
open class MuseStreamHsi: IXNMuseDataListener {
  ...
  private var hsiBuffer: RingBuffer<UInt64>
...
//    private let bufferSize: Int = 1024 * 4 * MemoryLayout<Int>.size // 4 * 64b chanels @todo use struct instead
  ...
  public init?(muse: IXNMuse) {
    ...
    guard let hsiBuffer = RingBuffer<UInt64>(capacity: bufferCount * 5) else {
      // @todo throw insufficient memory exception
      print("not enough memory")
      return nil
    }
    self.hsiBuffer = hsiBuffer
  }
  ...
  func start() {
    ...
    muse.register(self, type: IXNMuseDataPacketType.hsi)
  }
  
  func stop() {
    muse.unregisterDataListener(self, type: IXNMuseDataPacketType.hsi)
  ...
  public func receive(_ packet: IXNMuseDataPacket?, muse: IXNMuse?) {
    ...
    case IXNMuseDataPacketType.hsi:
      ...
      guard self.hsiBuffer.availableCapacity >= 5 else {
      ...
      self.hsiBuffer.feed(UInt64.init(bitPattern: packet!.timestamp()))
      self.hsiBuffer.feed(val[IXNEeg.EEG1.rawValue].doubleValue.bitPattern)
      self.hsiBuffer.feed(val[IXNEeg.EEG2.rawValue].doubleValue.bitPattern)
      self.hsiBuffer.feed(val[IXNEeg.EEG3.rawValue].doubleValue.bitPattern)
      self.hsiBuffer.feed(val[IXNEeg.EEG4.rawValue].doubleValue.bitPattern)
  ...
  public func isEmpty() -> Bool {
    return self.hsiBuffer.count == 0
  }
  public func consume() -> IXNMuseDataPacket? {
    guard self.hsiBuffer.count >= 5 else {
      return nil
    }

    let tail = self.hsiBuffer.tail!
    ...
    self.hsiBuffer.free(5)

    return IXNMuseDataPacket.make(IXNMuseDataPacketType.hsi, timestamp: timestamp, values: values)
  }
}
