...
open class MuseSensorEeg: AWARESensor {
  static private let sensorName: String = "muse_eeg"
  static private let entityName: String = "EntityMuseEeg"
  
  private var isRecording: Bool = false
  var stream: MuseStreamEeg?
  ...
  public override init(awareStudy: AWAREStudy, dbType: AwareDBType) {
    ...
    let storage: AWAREStorage = SQLiteStorage.init(study: awareStudy, sensorName: MuseSensorEeg.sensorName, entityName: MuseSensorEeg.entityName, dbHandler: ExternalCoreDataHandler.shared(), insertCallBack: insertEntityCallback)

    // Prevent aware internal buffer as we already use our own ring buffer
    storage.setBufferSize(1)
    
    // Call new official initializer
    super.init(awareStudy: awareStudy, sensorName: MuseSensorEeg.sensorName, storage: storage)
  }
  ...
  open func startRecording(muse: IXNMuse) {
    do {
      ...
      self.stream = MuseStreamEeg(muse: muse)
  ...
  open override func createTable() {
    if self.isDebug() {
      os_log("[%@] Create Table", MuseSensorEeg.sensorName);
    }
