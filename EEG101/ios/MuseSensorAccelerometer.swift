...
open class MuseSensorAccelerometer: AWARESensor {
  static private let sensorName: String = "muse_accelerometer"
  static private let entityName: String = "EntityMuseAccelerometer"
  
  private var isRecording: Bool = false
  var stream: MuseStreamAccelerometer?
  ...
  public override init(awareStudy: AWAREStudy, dbType: AwareDBType) {
    ...
    let storage: AWAREStorage = SQLiteStorage.init(study: awareStudy, sensorName: MuseSensorAccelerometer.sensorName, entityName: MuseSensorAccelerometer.entityName, dbHandler: ExternalCoreDataHandler.shared(), insertCallBack: insertEntityCallback)

    // Prevent aware internal buffer as we already use our own ring buffer
    storage.setBufferSize(1)
    
    // Call new official initializer
    super.init(awareStudy: awareStudy, sensorName: MuseSensorAccelerometer.sensorName, storage: storage)
  }
  ...
  open func startRecording(muse: IXNMuse) {
    do {
      ...
      self.stream = MuseStreamAccelerometer(muse: muse)
  ...
  open override func createTable() {
    if self.isDebug() {
      os_log("[%@] Create Table", MuseSensorAccelerometer.sensorName);
    }
