...
open class MuseSensorHsiPrecision: AWARESensor {
  static private let sensorName: String = "muse_hsiprecision"
  static private let entityName: String = "EntityMuseHsiPrecision"
  
  private var isRecording: Bool = false
  var stream: MuseStreamHsiPrecision?
  ...
  public override init(awareStudy: AWAREStudy, dbType: AwareDBType) {
    ...
    let storage: AWAREStorage = SQLiteStorage.init(study: awareStudy, sensorName: MuseSensorHsiPrecision.sensorName, entityName: MuseSensorHsiPrecision.entityName, dbHandler: ExternalCoreDataHandler.shared(), insertCallBack: insertEntityCallback)

    // Prevent aware internal buffer as we already use our own ring buffer
    storage.setBufferSize(1)
    
    // Call new official initializer
    super.init(awareStudy: awareStudy, sensorName: MuseSensorHsiPrecision.sensorName, storage: storage)
  }
  ...
  open func startRecording(muse: IXNMuse) {
    do {
      ...
      self.stream = MuseStreamHsiPrecision(muse: muse)
  ...
  open override func createTable() {
    if self.isDebug() {
      os_log("[%@] Create Table", MuseSensorHsiPrecision.sensorName);
    }
