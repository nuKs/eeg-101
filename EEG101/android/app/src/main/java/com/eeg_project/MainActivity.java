...
public class MainActivity extends ReactActivity {
    ...
    // @warning Methods such as #resumeOnPermissionsGranted() can be called before react is
    // initiated, implying the activity/intent will switch once react is loaded - this will crash
    // aware because the used #getApplicationContext() will change while the study is loading
    // asynchronously. This is why we rely on an inner ReactActivityDelegate class for callbacks !
    ...
    protected class MainActivityDelegate extends ReactActivityDelegate implements AwareStartListener {
        private final MainActivity _this;
        private boolean _awareStartHasBeenRequested = false;

        public MainActivityDelegate(Activity activity, @Nullable String mainComponentName) {
            super(activity, mainComponentName);
            _this = (MainActivity) activity;
            AwareModule.registerAwareStartListener(this);
        }

        @Override
        public void onAwareStartRequested() {
            Log.i("MainActivity", "_awareStartHasBeenRequested = true;");
            _awareStartHasBeenRequested = true;

            // Request permissions & launch aware.
            if (!checkPermissions()) {
        ...
        protected boolean checkPermissions() {
            ...
            for (String p : REQUIRED_PERMISSIONS) { //loop to check all the required permissions.
                if (PermissionChecker.checkSelfPermission(_this, p) != PermissionChecker.PERMISSION_GRANTED) {
                    return false;
                }
            }
        ...
        protected void requestPermissionsAndResumeOnGranted() {
            ...
            requestPermissions(REQUIRED_PERMISSIONS, REQUEST_CODE, null); // requestCode number has been copied from com.aware.ui.PermissionsHandler, I don't know what it does, I think it's arbitrary to filter results in callback #onRequestPermissionsResult
                ...
                timer.schedule(new TimerTask() {
                    ...
                    public void run() {
                        ...
                        */
                        ...
                        Aware.setSetting(getApplicationContext(), Aware_Preferences.WEBSERVICE_SILENT, false); //don't show notifications of synching events
                        ...
                        Aware.setSetting(getApplicationContext(), com.eeg_project.aware_plugins.eegmuse.Settings.STATUS_PLUGIN_EEGMUSE, true);
        ...
        protected void onResume() {
            ...
            // Ensure permission requests & aware is started once activation has been done.
            if (_awareStartHasBeenRequested) {
